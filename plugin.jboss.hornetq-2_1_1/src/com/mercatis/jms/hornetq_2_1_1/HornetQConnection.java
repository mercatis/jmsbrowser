package com.mercatis.jms.hornetq_2_1_1;
...
public class HornetQConnection extends
		AbstractJmxJmsConnection<ConnectionFactory, org.hornetq.jms.client.HornetQConnection, org.hornetq.jms.client.HornetQSession> {
	...
	protected void preOpen(Map<String, String> properties) throws JmsBrowserException {
		...
		try {
			...
			throw new JmsBrowserException("error getting context for hornetq connectin.", e);
		}
	}

	/*
	 * to use this feature put a new property with name "Management Queue" to the plugin.xml extensions with default value "hornetq.management"
	 */
	@Override
	...
	public List<JmsDurableSubscription<?>> getDurableSubscriptions(String topicName) throws JmsBrowserException {
		if (isJmxUsable()) {
			try {
				...
				List<JmsDurableSubscription<?>> result = new ArrayList<>(infos.length);
				for (SubscriptionInfo info : infos) {
					result.add(new JmsDurableSubscription<>("jms.topic." + topicName, info.getName(), info.getClientID(), info.getQueueName()));
				}
				return result;
			} catch (Exception e) {
	...
	protected int getSubscriberMessageCount(JmsDurableSubscription<?> subscriberInfo) throws JmsBrowserException {
		if (isJmxUsable()) {
			...
			try {
				...
				JMSQueueControl queueControl = (JMSQueueControl) getMBean(on, JMSQueueControl.class, false);
				return queueControl.countMessages(null); //(sub.clientID, sub.name, "");
			} catch (Exception e) {
				e.printStackTrace();
			}
// this also works
